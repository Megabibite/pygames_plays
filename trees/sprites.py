...
# 2:
# Define the missing glCreateShader method
# _glCreateShader = None
# def glCreateShader( shadertype ):
    # global _glCreateShader
    # if not _glCreateShader:
        # import ctypes
        # # Open the opengl32.dll
        # gldll = ctypes.windll.opengl32
        # # define a function pointer prototype of *(GLuint program, GLenum pname, GLint value)
        ...
        # # Get the win gl func adress
        # fptr = gldll.wglGetProcAddress( 'glCreateShaderARB' )
        # if fptr==0:
            # raise Exception( "wglGetProcAddress('glCreateShaderARB') returned a zero adress, which will result in a nullpointer error if used.")
        # _glCreateShader = prototype( fptr )
